...
const showResult = (result, quiz) => {
  ...
  block.innerHTML = `
  <h2 class="main_subtitle main__subtitle_result">Ваш результат</h2>
...
const renderQuiz = quiz => {
  ...
  const showQuestion = () => {
    ...
    fieldset.append(legend, ...answerData.labels);
    ...
    form.addEventListener('submit', (event) => {
      ...
      if (ok) {
        ...
        if (answer.every((result, i) => !!result === answerData.keys[i]
        )) {
          result += 1;
        };
